document.addEventListener('DOMContentLoaded', () => {
    ...
    const bandHBreakdown = { standard: 125000, single: 16000, disregarded: 2000, empty: 2000, second: 5000 };
    const totalPropertiesInBreakdown = Object.values(bandHBreakdown).reduce((a, b) => a + b, 0);
    const weightedCharge = (
        bandHBreakdown.standard * 1.0 + bandHBreakdown.single * 0.75 + bandHBreakdown.disregarded * 0.50 +
        bandHBreakdown.empty * 2.0 + bandHBreakdown.second * 2.0
    );
    ...
    // --- Formatting Helpers ---
    const formatToMillions = (num) => `£${Math.round(num / 1000000).toLocaleString()}m`;
    const formatToCurrency = (num) => `£${Math.round(num).toLocaleString()}`;
    const formatSliderTooltip = (value) => `£${(value / 1000000).toFixed(2)}m`.replace('.00', '');
    ...
    const dom = {
        ...
        percentTypeRadios: document.querySelectorAll('input[name="percent-type"]'),
        bandSlider: document.getElementById('band-slider'),
        
        bands: Array.from({ length: 4 }, (_, i) => {
            const slider = document.querySelector(`.rate-slider[data-band="${i}"]`);
    ...
    function update() {
        ...
        for (let i = 0; i < dom.bands.length; i++) {
            ...
            const propertyCount = estimateHighValueProperties(boundaries[i], boundaries[i+1]);
            let bandRevenue = 0;
            let avgIncrease = 0;
    ...
    const avgValue = estimateAverageValueInBand(boundaries[i], boundaries[i+1]);
    const rates = dom.bands.map(b => parseFloat(b.slider.value) / 100);
    let marginalTax = 0;
    for (let k = 0; k < i; k++) {
        const lower = boundaries[k];
        const upper = boundaries[k+1];
        const width = Math.max(0, Math.min(upper, avgValue) - lower);
 ...
 avgIncrease = totalAvgTax - BASE_BAND_H_TAX;
    ...
    function handleModeChange() {
        ...
        dom.rateHeader.textContent = isMultiplier ? 'Multiplier (vs Band D)' : 'Tax Rate (%)';
        ...
        dom.bands.forEach((band, index) => {
            ...
            if (isMultiplier) {
                slider.min = 2; slider.max = 12; slider.step = 0.1;
                band.rateSuffix.textContent = 'x';
            } else {
                slider.min = 0.0; slider.max = 5.0; slider.step = 0.1;
    ...
    function getPropertyDistribution() {
        ...
        const original_total_over_1_5m = 10600.0, original_cumulative_counts = [0, 5100, 8200, 9200, 9600, 10300], original_count_over_10m = 300.0;
        const scaling_factor = TOTAL_PROPERTIES_OVER_1_5M / original_total_over_1_5m;
        const scaled_cumulative_counts = original_cumulative_counts.map(c => c * scaling_factor), scaled_total_count_over_10m = original_count_over_10m * scaling_factor;
        const known_prices_m = [1.5, 2.0, 3.0, 4.0, 5.0, 10.0], alpha = 1.736966;
        const log_prices = known_prices_m.map(p => Math.log(p)), log_counts = scaled_cumulative_counts.map(c => Math.log1p(c));
        const get_cumulative_count_at_price = (price_gbp) => {
            price_gbp = Math.min(price_gbp, MAX_PRICE);
            const price_m = price_gbp / 1000000;
            if (price_m <= 10.0) return Math.expm1(linearInterpolate(Math.log(price_m), log_prices, log_counts));
            else {
                const max_price_m = MAX_PRICE / 1000000;
                const numerator = (10.0 ** -alpha) - (price_m ** -alpha), denominator = (10.0 ** -alpha) - (max_price_m ** -alpha);
                return scaled_cumulative_counts[scaled_cumulative_counts.length - 1] + scaled_total_count_over_10m * (numerator / denominator);
            }
        };
    ...
    function estimateHighValueProperties(lower_bound, upper_bound) {
        ...
        return get_cumulative_count_at_price(upper_bound) - get_cumulative_count_at_price(lower_bound);
    }
    ...
    function estimateAverageValueInBand(lower_bound, upper_bound) {
        ...
        lower_bound = Math.max(lower_bound, BAND_H_START_VALUE);
        upper_bound = isFinite(upper_bound) ? upper_bound : MAX_PRICE;
        const totalProperties = estimateHighValueProperties(lower_bound, upper_bound);
        if (totalProperties < 1) return (lower_bound + upper_bound) / 2;
        let totalValue = 0;
        const steps = 100, stepSize = (upper_bound - lower_bound) / steps;
        for (let i = 0; i < steps; i++) {
            const slice_low = lower_bound + (i * stepSize), slice_high = slice_low + stepSize;
            totalValue += estimateHighValueProperties(slice_low, slice_high) * (slice_low + (stepSize / 2));
        }
...
(function(){
  ...
  document.addEventListener('click', (e) => {
    ...
    document.querySelectorAll('.band-popover:not([hidden])').forEach(p => p.setAttribute('hidden',''));
    ...
    const bandName = row.querySelector('.band-name')?.textContent.trim() || `H${idx+1}`;
    const rangeCell = row.querySelector('td:nth-child(2)');
    const rangeText = rangeCell ? rangeCell.textContent.replace(/\s+/g,' ').trim() : '';
    const rateVal = row.querySelector('.rate-val')?.textContent.trim() || '';
    const rateSuffix = row.querySelector('.rate-suffix')?.textContent.trim() || '';
    const props = row.querySelector(`#h${idx+1}-props`)?.textContent.trim() || '—';
    const avg = row.querySelector(`#h${idx+1}-avg-tax`)?.textContent.trim() || '—';
    const addl = row.querySelector(`#h${idx+1}-revenue`)?.textContent.trim() || '—';
    ...
    pop.addEventListener('click', () => pop.setAttribute('hidden',''), { once: true });
  });
})();
